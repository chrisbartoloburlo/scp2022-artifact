...
object ClientLogic {
  def apply(server: In[M220], pathname: String, experimentType: String, iterations: Int, run: Int)(implicit ec: ExecutionContext, timeout: Duration) {
    val l = new logger(f"${pathname}/${experimentType}_${iterations}_exec_time_run${run}.csv")
